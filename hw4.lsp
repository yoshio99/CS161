...
(defun node2var (n c k)
  (+ (* (- n 1) k) c) ; uses the formula given in the specification to convert the node to variable
  )
...
(defun at-least-one-color (n c k)
  (cond ((= c k) (list (node2var n c k))) ; if c == k, no more color indexes to check
    (t (cons (node2var n c k) (at-least-one-color n (+ c 1) k)) ; convert node at color index to variable, then recursively call at-least-one-color with next color index
    )
  )
  )
...
(defun at-most-one-color (n c k)
  (cond ((= c k) nil) ; if c == k, traversal is finished
  (t (make-clauses n (+ c 1) k c) ; else, call helper function on next color index
  ))
)

; helper function for at-most-one-color, which actually constructs the clauses
(defun make-clauses (n c k orig)
  (cond ((= orig k) nil) ; if orig == k, traversal is finished
    ((= c k) (cons (append (list (- (node2var n orig k)) (- (node2var n c k)))) (make-clauses n (+ orig 2) k (+ orig 1)))) ; else if c == k, finished comparing c to all color indexes > c, move to next color index
    (t (cons (append (list (- (node2var n orig k)) (- (node2var n c k)))) (make-clauses n (+ c 1) k orig)) ; else, make clause and move to next comparison (orig vs. c+1)
    )
  )
)
...
(defun generate-node-clauses (n k)
  (cons (at-least-one-color n 1 k) (at-most-one-color n 1 k)) ; combine results of at-least-one-color and at-most-one-color
  )
...
(defun generate-edge-clauses (e k)
  (generate-edges e 1 k) ; call helper function
)

; helper function for generate-edge clauses, actually generates the edge clauses
(defun generate-edges (e c k) 
  (cond ((= c k) (cons (append (list (- (node2var (car e) c k)) (- (node2var (cadr e) c k)))) nil)) ; if c == k, return final clause for c
  (t (cons (append (list (- (node2var (car e) c k)) (- (node2var (cadr e) c k)))) (generate-edges e (+ c 1) k)) ; else, append edge clause to recursive call of generate-edges on next color
  ))
)
